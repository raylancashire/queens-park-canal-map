...
-   `analysis.html` --- sample analysis, participation analysis and
    water-pollution incident register
-   `freshwater.csv` --- the water-quality sampling data
-   `water-pollution-incidents.csv` --- optional/fallback
    incident-register data
...
Nitrate and phosphate results are displayed with a plain-language
...
The assessment is calculated automatically from the midpoint values
supplied in the CSV.

Turbidity is displayed separately as a clarity result. Where
appropriate, a descriptive label such as **Very clear** is shown.
...
field-name mappings in `index.html` may need to be updated.

------------------------------------------------------------------------

## Update --- 15 September 2026

A new **Water pollution incidents** register has been added to
`analysis.html` so pollution incidents, suspected pollution and
environmental events can be recorded separately from routine FreshWater
Watch sample results.
...
-   Support for optional latitude and longitude fields in incident
    records.
-   A separate Supabase table, `water_pollution_incidents`, for the live
    incident register.
-   A CSV fallback using `water-pollution-incidents.csv` where the
    Supabase incident table is not available.

### Incident administration

Incident editing is restricted to an authenticated administrator
account. Public visitors can view the incident register but cannot
change it.
...
-   Automatically close the incident form after saving, cancelling or
    deleting.
...
The wording allows uncertain information to be recorded appropriately,
for example by describing a source as **suspected** or **believed**
rather than confirmed.

### Batch CSV import

The signed-in **Import CSV** panel now includes a **Download CSV
template** button.

The template is generated directly by `analysis.html`, helping to keep
the batch-import format aligned with the current incident form. It
includes the current incident fields plus a guidance row showing
...
Records are checked before import, including required fields and
duplicate handling.

### Initial historic incident

The register has been tested with the **29 February 2024 Grand Union
Canal cooking-oil pollution incident**, recorded as a pollution incident
affecting the canal from Alperton towards Little Venice / Paddington
Basin. The incident record demonstrates the use of the expanded fields
for official category, suspected source, extent, response, wildlife
impact and organisations involved.

### Authentication improvements

The incident administration interface now uses the existing Supabase
authenticated account rather than relying on a separate magic-link
workflow for each incident-management session. This provides a clearer
signed-in state and avoids the redirect and email-rate-limit problems
encountered during initial testing.

These additions keep routine citizen-science sampling results and
exceptional pollution events distinct while allowing both to be analysed
within the wider Canal Watch project.

### Incident form validation and UK dates

The incident administration form has also been improved for clearer data
entry:

-   Required fields are highlighted when information is missing.
-   A short validation message identifies what needs correcting.
-   Incident and resolved dates accept the public-facing UK format
    `DD/MM/YYYY`.
-   Dates are validated before conversion to the ISO format used by
    Supabase.
-   Existing incident dates are shown in UK format when a record is
    edited.
-   The incident form closes automatically after saving, cancelling or
    deleting.

### Incident photographs

Incident records can now include an optional photograph.

-   A photograph can be selected directly from the Add / Edit Incident
    form.
-   Images are resized in the browser before upload, with a maximum
    dimension of approximately 1200 pixels and JPEG compression to
    reduce storage and transfer size.
-   A preview is shown before saving.
-   Photographs are stored in the private Supabase Storage bucket
    `incident-photos`.
-   The incident record stores the Storage reference in `photo_url`.
-   Signed URLs are generated when a private image needs to be
    displayed.
-   Existing photographs are retained when editing unless they are
    deliberately replaced or removed.
-   Photographs can be replaced or removed from an existing incident.
-   Deleting an incident also removes its associated stored photograph
    where possible.
-   A thumbnail is shown in the incident register.
-   The same photograph is displayed in the pollution-incident map
    popup.

The `incident-photos` Storage bucket uses policies that permit public
viewing through the application while restricting upload, update and
deletion to authenticated users. If the authentication model is expanded
to ordinary volunteer accounts in future, these write policies should be
reviewed and tightened.

### Pollution incidents on the map

`index.html` can display pollution incidents that have valid latitude
and longitude coordinates.

Current incident-map features include:

-   A **Show pollution incidents** switch.
-   Incident markers loaded from the Supabase
    `water_pollution_incidents` table.
-   Popups showing the incident location, date, type, severity, official
    category, status, extent, suspected source, wildlife impact,
    organisations, description and response / action.
-   The incident photograph at the top of the popup when one is
    available.
-   Automatic refresh of the pollution-incident marker layer after an
    incident is saved, edited or deleted in `analysis.html`.
-   Marker refresh does not recreate the Leaflet map, so the visitor's
    current map centre and zoom are retained.

The Analysis page and map coordinate the refresh using browser storage /
`BroadcastChannel`, allowing the incident layer to update without
requiring a full page reload.

### Severity-coloured incident markers

Pollution incidents retain a consistent circular map marker so they
remain visually recognisable as the same class of feature. Both the
marker colour and the symbol inside the circle communicate the recorded
**Severity**, improving accessibility by avoiding reliance on colour
alone.

  Recorded severity          Marker colour   Internal symbol
  -------------------------- --------------- -----------------
  Low / Minor                Yellow          `i`
  Moderate / Medium          Orange          `!`
  High / Major               Red             `!!`
  Severe / Critical          Red             `⚠`
  Not rated / unrecognised   Grey            `?`

The popup continues to show the written severity value, so the symbol is
an additional visual cue rather than a replacement for text.

### Multiple incidents at one location

Each pollution event remains a separate incident record even when
several incidents use the same location or coordinates. This preserves
the date, type, severity, status, photograph and other details for each
individual event. Where coordinates are identical, the current map can
therefore contain overlapping incident markers; grouping repeated
incidents into a single location-history popup can be considered as a
future enhancement.
